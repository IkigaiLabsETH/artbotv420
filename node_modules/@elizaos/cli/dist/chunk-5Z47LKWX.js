...
var require_brace_expansion = __commonJS({
  "../../node_modules/glob/node_modules/minimatch/node_modules/brace-expansion/index.js"(exports, module) {
...
// src/commands/env.ts
var CONFIG_FILE = path2.join(os.homedir(), ".eliza", "config.json");
async function getCustomEnvPath() {
  try {
    if (!existsSync(CONFIG_FILE)) {
      return null;
    }
    const content = await fs2.readFile(CONFIG_FILE, "utf-8");
...
async function saveCustomEnvPath(customPath) {
  try {
...
async function getGlobalEnvPath() {
  ...
  return path2.join(elizaDir, ".env");
}
function getLocalEnvPath() {
  const localEnvPath = path2.join(process.cwd(), ".env");
...
async function listEnvVars() {
  ...
  const customPath = await getCustomEnvPath();
  const globalEnvLabel = customPath ? `Global environment variables (custom path: ${customPath})` : "Global environment variables (.eliza/.env)";
  logger.info(base_exports.bold(`
${globalEnvLabel}:`));
  if (Object.keys(globalEnvVars).length === 0) {
    ...
    Object.entries(globalEnvVars).forEach(([key, value]) => {
      logger.info(`  ${base_exports.green(key)}: ${maskedValue(value)}`);
    });
  }
  if (localEnvPath) {
...
async function editEnvVars(scope, fromMainMenu = false) {
  const envPath = scope === "global" ? await getGlobalEnvPath() : getLocalEnvPath();
  if (scope === "local" && !envPath) {
    const { createLocal } = await (0, import_prompts.default)({
      type: "confirm",
      name: "createLocal",
    ...
    await writeEnvFile(path2.join(process.cwd(), ".env"), {});
...
Current ${scope} environment variables:`));
  if (Object.keys(envVars).length === 0) {
    ...
    const { addNew } = await (0, import_prompts.default)({
      type: "confirm",
      name: "addNew",
  ...
  while (!exit) {
    ...
    const { selection } = await (0, import_prompts.default)({
      type: "select",
      name: "selection",
...
async function addNewVariable(envPath, envVars) {
  const { key } = await (0, import_prompts.default)({
    ...
    validate: (value2) => value2.trim() !== "" ? true : "Variable name cannot be empty"
  });
  if (!key) return;
  const { value } = await (0, import_prompts.default)({
    type: "text",
...
async function resetEnv() {
  ...
  const elizaDir = path2.join(homeDir, ".eliza");
  const globalEnvPath = await getGlobalEnvPath();
  const cacheDir = path2.join(elizaDir, "cache");
  const dbDir = path2.join(elizaDir, "db");
  if (existsSync(globalEnvPath)) {
    await fs2.unlink(globalEnvPath);
  ...
  const { resetDb } = await (0, import_prompts.default)({
    type: "confirm",
    name: "resetDb",
...
async function setEnvPath(customPath) {
  const resolvedPath = path2.resolve(customPath);
  const isDirectory = existsSync(resolvedPath) && (await fs2.stat(resolvedPath)).isDirectory();
  let finalPath = resolvedPath;
  if (isDirectory) {
    finalPath = path2.join(resolvedPath, ".env");
    logger.info(`Path is a directory. Will use ${finalPath} for environment variables.`);
  }
  const parentDir = path2.dirname(finalPath);
  ...
  if (!existsSync(finalPath)) {
    const { createFile } = await (0, import_prompts.default)({
      type: "confirm",
      name: "createFile",
...
var env = new Command().name("env").description("Manage environment variables and secrets");
env.command("list").description("List all environment variables").action(async () => {
  try {
    await listEnvVars();
  } catch (error) {
    handleError(error);
...
env.command("edit-global").description("Edit global environment variables").action(async () => {
  try {
...
env.command("edit-local").description("Edit local environment variables").action(async () => {
  try {
...
env.command("reset").description("Reset all environment variables and wipe the cache folder").action(async () => {
  try {
    await resetEnv();
  } catch (error) {
    handleError(error);
...
env.command("set-path <path>").description("Set a custom path for the global environment file").action(async (customPath) => {
  try {
...
env.command("interactive").description("Interactive environment variable management").action(async () => {
  try {
    await showMainMenu();
  } catch (error) {
    handleError(error);
